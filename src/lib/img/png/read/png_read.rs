...
impl<'i> PNGImage<'i> {
  pub fn read(&mut self, path: &Path) -> Result<(), RSMError> {
    let mut file: File = File::open(path)?;
    let file_sz: usize = file.metadata()?.len() as usize;
    let pg_sz: usize = page_size::get();

    // Natively read file if contents are small
    if pg_sz * 4 < file_sz {
  ...
  pub fn read_bytes(&mut self, mut bytes: &[u8]) -> Result<(), RSMError> {
    bytes = bytes.trim_ascii();
    if bytes.is_empty() {
      return Err(RSMError::Empty);
    }
    let mut reader: PNGReader<'_> = PNGReader::new();
    let chunks: Vec<Chunk<'_>> = reader.read(bytes)?;
    println!("{:?}", chunks);
    Ok(())
  }
}
