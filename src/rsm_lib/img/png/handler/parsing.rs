...
impl<'b> PNGHandler<'b> {
  /// Reads every chunk to get all the information
  pub(super) fn parse_chunks(&mut self) -> Result<(), String> {
  ...
  /// Parse a single chunk to obtain its information
  fn parse_chunk(&mut self, chunk: Chunk) -> Result<(), String> {
    match chunk.r#type {
      ChunkType::IHDR => self.handle_IHDR(chunk),
      ChunkType::IDAT => self.handle_IDAT(chunk),
      ChunkType::IEND => self.handle_IEND(),
      _ => Ok(()),
    }
  }

  /// Handle the IDHR chunk in the png file
  fn handle_IHDR(&mut self, chunk: Chunk) -> Result<(), String> {
    if chunk.length != 13 {
      return Err(format!("Invalid IHDR length: {}", chunk.length));
    }
    let data: &[u8; 13] = &chunk.data.try_into().unwrap();
    let width: u32 = self.read_IHDR_size(&data[0..4], "width")?;
    let height: u32 = self.read_IHDR_size(&data[4..8], "height")?;
  ...
  /// Reads an IHDR chunk dimension (width or height)
  fn read_IHDR_size(&self, size: &[u8], name: &str) -> Result<u32, String> {
    let dim: u32 = u32::from_be_bytes(size.try_into().unwrap());
    if dim == 0 {
      return Err(format!("Invalid image {}: {}", name, dim));
    }
    Ok(dim)
  }

  /// Push the IDAT chunks into the IDAT vector
  fn handle_IDAT(&mut self, chunk: Chunk) -> Result<(), String> {
    self.idat_bytes.extend(chunk.data);
    Ok(())
  }

  /// Handle the read IDAT chunks
